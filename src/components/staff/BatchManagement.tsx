...
export function BatchManagement({ batches, onBatchesChange, currentUser }: BatchManagementProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^B\d+$/.test(cleanId)) {
      setError(
        "Format ID Batch tidak valid. Harus diawali huruf 'B' diikuti angka (contoh: B005)."
      );
      return;
    }
  ...
  const handleDelete = async (batchId: string) => {
    ...
    try {
      ...
      if (json.ok) {
        ...
        onBatchesChange((prev) =>
          prev.map((b) => (b.id === batchId ? { ...b, deleted: true } : b))
        );
  ...
  const inputCls =
    "w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-[#01696f] focus:ring-2 focus:ring-[#01696f]/10 transition-all disabled:bg-slate-100 disabled:text-slate-400";
    ...
    <div className="p-6 max-w-[1400px] w-full mx-auto flex flex-col gap-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Kelola data batch yang terhubung ke data registrasi pasien dan penyusunan laporan.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <form
          ...
          className="lg:col-span-4 bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm p-5 space-y-4"
        >
          ...
          <AnimatePresence mode="wait">
            {error && (
              <motion.div
                ...
                className="bg-red-50 text-red-600 text-xs p-3 rounded-lg border border-red-100 flex items-start gap-2"
              >
            ...
            {success && (
              <motion.div
                ...
                animate={{ opacity: 1, height: "auto" }}
                exit={{ opacity: 0, height: 0 }}
                className="bg-emerald-50 text-emerald-700 text-xs p-3 rounded-lg border border-emerald-100 flex items-start gap-2"
              >
            ...
            {rawImage ? (
              <div className="flex flex-col gap-3 p-4 border border-slate-200 rounded-xl bg-slate-50">
                <p className="text-[11px] font-semibold text-slate-700">
                  Geser dan tarik sudut-sudut kotak untuk memotong logo:
                </p>
                <div
                ...
                >
                  ...
                  >
                    {/* Grid lines */}
                    <div
                      className="absolute left-0 right-0 border-b border-white/30 pointer-events-none"
                      style={{ top: "33.33%" }}
                    />
                    <div
                      className="absolute left-0 right-0 border-b border-white/30 pointer-events-none"
                    ...
                    />
                    <div
                      className="absolute top-0 bottom-0 border-r border-white/30 pointer-events-none"
                      style={{ left: "33.33%" }}
                    />
                    <div
                      className="absolute top-0 bottom-0 border-r border-white/30 pointer-events-none"
              ...
              <div className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl bg-slate-50">
                <img
                  src={logo}
                  alt="Logo Preview"
                  className="w-12 h-12 object-contain bg-white rounded border border-slate-100 p-1 shrink-0"
                />
              ...
              <div className="relative group border border-dashed border-slate-300 rounded-xl p-4 text-center hover:bg-slate-50 hover:border-[#01696f]/40 transition-all cursor-pointer">
                ...
                <p className="text-xs text-slate-500 font-medium">
                  Klik untuk upload logo perusahaan
                </p>
          ...
          {useLogoInReport && (
            <div className="border border-slate-200 rounded-xl p-4 bg-slate-50/50 space-y-3">
              ...
              <div
                className="bg-white border border-slate-200 p-4 rounded-lg shadow-sm overflow-hidden select-none"
                style={{ width: "100%", fontFamily: "'Times New Roman', 'Georgia', serif" }}
              >
                <div className="flex items-center gap-3" style={{ fontSize: "9px" }}>
                  ...
                  <div className="flex-1 text-center shrink-0">
                    <p className="font-bold text-[9px] tracking-wider text-slate-800 m-0">
                      ASISYA PSYCHOLOGICAL CENTER
                    </p>
                    <p className="text-[6px] text-slate-500 m-0 mt-0.5 leading-none">
                      Ruko Grand City Regency A7 - A8 Jl. Rungkut Madya
                    </p>
                    <p className="text-[6px] text-slate-500 m-0 mt-0.5 leading-none">
                      Surabaya - Jawa Timur
                    </p>
                  ...
                  {logo ? (
                    ...
                    <div className="w-10 h-10 shrink-0 border border-dashed border-slate-300 rounded flex items-center justify-center text-[7px] text-slate-400">
                      No Logo
                    </div>
                  )}
                </div>
                <hr
                  className="border-t border-slate-800 mt-2 mb-0"
        ...
        <div className="lg:col-span-8 bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          <h2 className="text-sm font-bold text-slate-800 border-b border-slate-100 px-5 py-4 bg-slate-50/60">
            Daftar Batch Terdaftar
          </h2>
