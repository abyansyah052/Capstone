...
function Modal({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`flex items-center justify-between px-5 py-4 border-b ${danger ? "border-red-100 bg-red-50/60" : "border-slate-100"}`}
        >
          <p className={`text-[14px] font-semibold ${danger ? "text-red-700" : "text-slate-900"}`}>
            {title}
          </p>
          <button
            onClick={onClose}
  ...
  return (
    <Modal title="Folder Baru" onClose={onClose}>
      <div className="px-5 py-4">
        ...
        />
        {name.length > 100 && <p className="mt-1 text-[11px] text-slate-400">{name.length}/128</p>}
      </div>
      <div className="flex items-center justify-end gap-2 px-5 py-3 bg-slate-50 border-t border-slate-100">
        <button
          onClick={onClose}
          className="px-4 py-2 text-sm text-slate-500 hover:text-slate-800 rounded-lg hover:bg-slate-100 transition-all"
        >
          Batal
        </button>
        <button
    ...
    <Modal title="Konfirmasi Hapus" onClose={onClose} danger>
      <div className="px-5 py-4 space-y-2">
        <p className="text-sm text-slate-700">
          Hapus {label} <span className="font-semibold text-slate-900 break-all">“{name}”</span>?
        </p>
        ...
        <div className="pb-1">
          {sortOptions.map((o) => (
            ...
            >
              <span>{o.label}</span>
              {sort === o.key && (
                <span className="w-1.5 h-1.5 flex-shrink-0 rounded-full bg-[#01696f]" />
              )}
            </button>
          ))}
        </div>
...
export function LaporanPsikologis({
  ...
  const [ctxMenu, setCtxMenu] = useState<{ menu: CtxMenu; pos: { x: number; y: number } } | null>(
    null
  );
    ...
    <>
      <div className="px-4 sm:px-6 py-6 max-w-5xl mx-auto">
        ...
        <nav
          ...
          className="flex flex-wrap items-center gap-0.5 text-sm text-slate-500 mb-5 min-w-0"
        >
          {path.length > 0 && (
            <button
        ...
        <div className="mb-4 flex gap-2 items-center">
          <div className="flex h-11 flex-1 items-center rounded-full border border-[#d9d6d0] bg-white px-4 gap-2.5 transition-all focus-within:border-[#01696f]/40 focus-within:ring-2 focus-within:ring-[#01696f]/10">
            ...
            {search && (
              <button
                onClick={() => setSearch("")}
                className="flex-shrink-0 rounded-full p-0.5 text-slate-400 hover:text-slate-600 hover:bg-slate-200 transition-colors"
          ...
          <div className="relative flex-shrink-0">
            ...
            >
              ...
              {activeFilterCount > 0 && (
                <span
                  className="flex items-center justify-center rounded-full bg-[#16254c] text-white text-[10px] font-bold leading-none"
          ...
          {filteredNodes.length === 0 ? (
            ...
            filteredNodes.map((node) => (
              <NodeRow
                ...
                pathLabel={
                  isDeepSearch
                    ? (() => {
                        ...
                        return getPathFolders(tree, hit.parentPath)
                          .map((f) => f.name)
                          .join(" / ");
                      })()
                    : undefined
                ...
                onMenuOpen={(e) =>
                  openCtxMenu({ nodeId: node.id, nodeName: node.name, nodeKind: node.kind }, e)
                }
      ...
      <AnimatePresence>
        {newFolderOpen && (
          <NewFolderModal onClose={() => setNewFolderOpen(false)} onCreate={createFolder} />
        )}
        ...
        {bulkDeleteOpen && (
          <Modal
            title={`Hapus ${selected.size} Item`}
            onClose={() => setBulkDeleteOpen(false)}
            danger
          >
            <div className="px-5 py-4 space-y-1.5">
              <p className="text-sm text-slate-700">
                Hapus <span className="font-semibold text-slate-900">{selected.size} item</span>{" "}
                yang dipilih?
              </p>
