  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (invalidFields.length > 0) {
      ...
      if (firstInvalidKey && errs[firstInvalidKey]) {
        alert(
          `Data tidak valid pada kolom ${labelMap[firstInvalidKey] || String(firstInvalidKey)}:\n${errs[firstInvalidKey]}`
        );
              ...
              <Field label="Usia" hint="Otomatis">
                <div className="relative flex items-center">
                  <input
                    ...
                    value={
                      age !== null
                        ? `${age} tahun${form.dateOfBirth ? "  ·  " + formatDate(form.dateOfBirth) : ""}`
                        : ""
                    }
          ...
          >
            <Field label="Pilih Batch" id="batchId">
              <div className="relative">
                ...
                >
                  ...
                  {batches
                    .filter((b) => !b.deleted || b.id === form.batchId)
                    .map((b) => (
            ...
            <AnimatePresence>
              {selectedBatch && (
                ...
                >
                  <div
                    className="flex items-center gap-3 px-4 py-3 rounded-lg border mt-1"
                    style={{
                      backgroundColor: selectedBatch.color + "10",
                      borderColor: selectedBatch.color + "30",
                    }}
                  >
                    <span
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-[0_1px_3px_rgba(15,23,42,0.04)]">
            ...
            <div className="p-5 flex flex-col gap-3 items-center">
              ...
              <p className="text-[11px] text-slate-400 text-center">
                JPG, PNG · Maks. 5 MB
                <br />
                Opsional — untuk identifikasi internal
              </p>
            </div>
          </div>
                  ...
                  >
                    <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-red-50 border border-red-100">
                      ...
                      <p className="text-[11px] text-red-600 font-medium leading-relaxed">
                        Terdapat {Object.values(errors).filter(Boolean).length} field yang perlu
                        diperbaiki sebelum menyimpan.
                      </p>
                    </div>
                  </motion.div>
                )}
